...
pdf(file = "~/Desktop/Harvard_NLP_Student/lessons/G_clustering/data/news_cluster_topics.pdf", 
    width  = 6, 
    height = 6) 
...
tidyCorp$source <- cut(as.numeric(tidyCorp$document), 
                       ...
                       labels = sourceID[,1])
idx <- (grep('101', tidyCorp$document)[1]-
  2):(grep('101', tidyCorp$document)[1]+2)
tidyCorp[idx,]
...
# Intersect the Clusters and Sentiment; subset to one topic cluster
...
# Intersect the Clusters and Sentiment; subset to one source and sum the sentiment
sourceID[5,1]
oneSource <- subset(combinedData, combinedData$source== sourceID[5,1])
...
ggplot(oneSource, aes(sentiment, as.factor(clusterAssignment), 
  ...
  geom_point() +
  ggtitle(sourceID[5,1], sub = "Emotion by Topic Cluster") + ylab("") +
  theme_tufte()

# Now let's rerun with MSNBC
sourceID[2,1]
...
ggplot(oneSource, aes(sentiment, as.factor(clusterAssignment), 
                      size = count, alpha = count)) +
  geom_point() +
  ggtitle(sourceID[2,1], sub = "Emotion by Topic Cluster") + ylab("") +
  theme_tufte()
